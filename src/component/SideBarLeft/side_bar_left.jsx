...
import ListLoading from "../../skeleton/list_loading";
        ...
        <React.Fragment>
            <div id="sidebar-left--container">
                <span>
                    {
                        loaderWeather === true ? (
                            <React.Fragment><div className="template-container">
                                ...
                                <ul className="list-forecast--weeks">
                                    {weatherForecast &&
                                        weatherForecast?.list?.reduce((acc, weatherWeek, index) => {
                                            ...
                                            const forecastDate = new Date(item.weather.dt_txt).getDay();
                                            const currentDay = new Date().getDay();

                                            // Only display future days
                                            if (forecastDate > currentDay) {
                                                return (
                                                    <li key={item.index} className="forecast-week--item">
                                                        <div className="row">
                                                            <h3 className="analyst">{days[forecastDate]}</h3>
                                                        ...
                                                        <div className="row">
                                                            <div className="analyst">
                                                                <img src={`http://openweathermap.org/img/wn/${item.weather.weather[0].icon}@2x.png`} alt="" /> {/* Sử dụng weather[0] để lấy icon */}
                                                                {item.weather.weather[0].description}
                                                            </div>
                                                            <div className="analyst">
                                                                <IoRainyOutline />
                                                                {item.weather.rain ? Object.values(item.weather.rain)[0] : 0}%
                                                            </div>
                                                            <div className="analyst">
                                                                <WiHumidity />
